...
function menorDeEdad(integrantesFamilia) {
  const existeMenor = integrantesFamilia.some(
    (integrante) => integrante.edad < 18
  );

  if (existeMenor) {
    return integrantesFamilia.filter((integrante) => integrante.edad < 18);
  }

  return "Esta familia no cuenta con ningun miembro menor a 18 años";
}

console.log(menorDeEdad(familia));
=======
